...
unsigned int period, duty,ratio;
...
static void cmd_period(BaseSequentialStream *chp, int argc, char *argv[]) {

  if (argc != 1) {
    chprintf(chp, "Usage: period value\r\n");
    return;
  }

  if(!((atoi(argv[0])>0)&&(atoi(argv[0])<10000))){
	chprintf(chp, "0 < value < 1000 !!\r\n");
	return;
  }
  
  period=atoi(argv[0]);
  pwmChangePeriod(&PWMD1, period);
  chprintf(chp, "period changed\r\n");
}

static void cmd_duty(BaseSequentialStream *chp, int argc, char *argv[]) {

  if (argc != 1) {
    chprintf(chp, "Usage: duty value\r\n");
    return;
  }

  if((atoi(argv[0]))>100){
	chprintf(chp, "value =< 100 !!\r\n");
	return;
  }
